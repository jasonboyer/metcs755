...
public class XmlInputFormat extends TextInputFormat {
	...
	//	        FileSystem fs = file.getFileSystem(conf);
		...
		public boolean nextKeyValue() throws IOException, InterruptedException {
			//logger.debug("XRR87^  "+fsin.getPos());
			if (fsin.getPos() < end) {
				if (readUntilMatch(startTag, false)) {
					//logger.debug("XRR90^  "+fsin.getPos());
					...
					try {
						...
						if (readUntilMatch(endTag, true)) {
							// Only take the stuff between the tags
							value.set(buffer.getData(), startTag.length, buffer.getLength() - startTag.length - endTag.length);
							//logger.debug("XRR97^key="+key.get()+" value="+value.toString());
							return true;
							...
							value.set(buffer.getData(), startTag.length, buffer.getLength() - startTag.length);
							logger.debug("XRR102^K="+key.get()+" V="+value.toString());
							return true;
						}
					} finally {
						buffer.reset();
